...
/** Yookassa create-session request — POST /payments/yookassa/create-session */
export interface CreateYookassaSessionDto {
  paymentDto: Pick<Payments.IPayment, 'amount' | 'description' | 'metadata' | 'status'>;
  userId: string;
  save_payment_method: boolean;
}

/** Common session response shape returned by both providers */
...
export namespace Payments {
  ...
  /**
   * DTO returned by the backend for `GET /payments/yookassa/saved-methods/:userId`.
   *
   * Mirrors the `SavedPaymentMethod` entity (bot-side view — only the fields
   * we actually consume on the client). Kept here instead of importing the
   * backend entity to avoid leaking TypeORM types into the bot.
   */

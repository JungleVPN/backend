...
describe('AutopaymentController', () => {
  ...
  const makePayload = (event: string): RemnawebhookPayload => ({
    event,
    data: { uuid: 'u-1', username: 'test', status: 'active', subscriptionUrl: '', telegramId: 42 },
    timestamp: new Date().toISOString(),
  });

  it('returns { received: true } for known events', async () => {
    const result = await controller.handleRemnaEvent(
      makePayload(REMNAWAVE_EVENTS.USER_EXPIRES_IN_24H),
    );
    expect(result).toEqual({ received: true });
  });

  it('delegates user.expires_in_24_hours to autopayment service (fire-and-forget)', async () => {
    const payload = makePayload(REMNAWAVE_EVENTS.USER_EXPIRES_IN_24H);
  ...
  it('returns { received: true } for unknown events without calling service', async () => {
    const result = await controller.handleRemnaEvent(makePayload('user.some_other_event'));
